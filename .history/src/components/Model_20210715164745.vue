 ...
 * @LastEditTime: 2021-07-15 16:47:45
    ...
    :style="{ height: '100%' }" >
     <van-cell icon="arrow-left" @click="cancelCityShow">
        ...
        <van-index-bar :index-list="indexList" highlight-color="#89BCD7" @select="indexOnSelect">
            ...
            <div v-for="(item,key) in 10"  :index="key">
                <van-cell>
                    <!-- 使用 title 插槽来自定义标题 -->
                    <template #title>
                        <van-image class="index_cell_url" :src="item"></van-image>
                        <span class="index_cell_name">{{item}}</span>
                    </template>
                </van-cell>
            </div>
        </van-index-bar>
...
import { ActionSheet,Form, Field, CellGroup,Button ,Cell,Toast,Popup,Divider,IndexBar} from 'vant';
