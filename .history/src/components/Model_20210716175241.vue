 ...
 * @LastEditTime: 2021-07-16 17:52:40
    ...
    <van-config-provider :theme-vars="themeVars">
        <van-action-sheet v-model:show="showOn" title="立即预约试驾">
            ...
            <div class="content_subhead" >提交信息后工作人员会在3个工作日内与您联系</div>
                <van-form v-if="isRate" class="from" @submit="onSubmit">
                    <van-cell-group>
                        <van-field
                        class="field"
                        v-model="state.city"
                ...
                <div v-else>
                    ...
                    <div >
                        <van-button  block  @click="onRate" class="btn">
                        提交
                        </van-button>
                    </div>
                </div>
        ...
        <van-cell icon="arrow-left" @click="cancelCityShow">
            ...
            <div class="city_auto_location">
                <button class="btn_city_content">{{state.city}}</button>
                <div class="replay_location">
                    <van-icon :class=" refreshLocation ===true?'city_location_icon':''" size="14px" name="replay" />
                    <button @click="getCityLocation">重新定位 </button>
                </div>
            </div>
            <!-- 索引 -->
            <van-index-bar :index-list="indexList" highlight-color="#89BCD7" @select="indexOnSelect">
                <van-index-anchor class="index_title"   index="定" v-show="false"></van-index-anchor>
                <div v-for="(item,key) in indexCityList"  :index="key">
                    <van-index-anchor :index="toUpperCase(item.initials)" />
                    <van-cell v-for="(item2,key) in item.city" :key="key" :title="item2.city_name" @click="onCitySelect(item2.city_name)" />
                </div>
            </van-index-bar>
        </van-popup>
    </van-config-provider>
...
<script lang="ts">
import { defineComponent ,onUnmounted,ref,reactive,onMounted} from 'vue'
import { ActionSheet,Form, Field, CellGroup,Button,Rate ,Cell,Toast,Popup,Divider,IndexBar,IndexAnchor} from 'vant';
import mitt, { Emitter } from "mitt";
import {isPhoneNumber} from '@utils/tool'
import { useRouter } from 'vue-router';
import {cityModel} from '@/models/cityModel'
import {showCityInfo} from '@/hooks/getCityLocation'
// 实例化mitt
...
export default defineComponent({
    ...
    setup(props,context) {
        ...
        //获取定位
        onMounted(async()=>{
            console.log(await showCityInfo())
            state.city = await showCityInfo()
        })    
       const router = useRouter()
        ...
        const onRate  =()=>{
            context.emit("onRate",rateValue.value)
        }
        // 添加mitt事件监听器事件及回调函数
    ...
    },
    {
        ...
        indexList.value.unshift("定")
        console.log(indexList.value)
       
        //小写转化大写
        const toUpperCase =(val:string):string=>{
